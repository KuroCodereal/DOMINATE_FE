...
export default function LoginForm() {
  ...
  async function onSubmit(data: RegisterFormValues) {
    startTransition(async () => {
      try {
        // --- Check exist trước khi gọi register ---
        const [checkEmailRes, checkUserNameRes, checkPhoneNumberRes] = await Promise.all([
          http.get<{ check: boolean }>(LINKS.check_email_exist, {
            params: { email: data.email },
            baseUrl: '/api',
        ...
        if (checkEmailRes.check || checkUserNameRes.check || checkPhoneNumberRes.check) {
          if (checkEmailRes.check) toast.error('Email already exists')
          if (checkUserNameRes.check) toast.error('Username already exists')
          if (checkPhoneNumberRes.check) toast.error('Phone number already exists')
          return
        }
        ...
        toast.error(msg || 'Register failed due to exception.')
      }
    })
  }
